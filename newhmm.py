...
def setProbForLetterDown(currentL, nextL):
    if currentL not in letterDict:
        return
    if nextL not in letterDict:
        return
...
while True:
    inp = input("Please input a word!")
    i = 0
    for char in inp:
        try:
            if getMaxProbForLetter(char, inp[i + 1]) < 0.07:
                print("The computer thinks that this word is not spelt correctly")
                ans = input("Is it correct?")
                if (ans == "y"):
                    prob = []
                    probNum = []
                    for let in letterProb.get(inp[i + 1]):
                        try:
                            probNum.append(letterDict.get(char)[let])
                            prob.append(let)
                        except:
                            continue
                    bestoption = prob[probNum.index(max(probNum))]
                    cp = bestoption[0]
                    print("[Best Option] Spelling error detected: did you mean " + inp.replace(inp[i+1], cp) + "?")
                    break
                elif (ans == "n"):
                    setProbForLetter(char, inp[i + 1])
                    print("The computer has learned")
            i += 1
        except:
            pass